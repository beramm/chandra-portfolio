...
- [x] Design direction: **motion-driven** (scroll-triggered animations, Framer Motion), monochrome + blue accent palette, Space Grotesk + Archivo fonts.
- [x] Dark mode: **yes** — light + dark toggle via `next-themes`, system preference default.
- [x] Contact method: **mailto link** (contact section on `/`, no form/API route).
...
- Admin pages (`/admin/*`) are behind `proxy.ts` (Next 16 middleware) cookie check; forms are Client Components (`"use client"`) posting to API routes.
...
3. **Auth** — request-otp / verify-otp API routes, MailerSend integration (OTP email), session cookie, middleware guard.
